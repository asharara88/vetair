...
export default async function CasePage({
  ...
  const [logsRes, msgsRes, roundsRes, reqRes, evalRes] = await Promise.all([
    ...
    supabase.from("country_rules")
      .select("id, requirement_code, title, requirement_type, priority, source_url")
      .eq("origin_country", caseRow.origin_country)
      .eq("destination_country", caseRow.destination_country)
      .eq("species", species)
      .eq("is_active", true),
    supabase.from("requirement_evaluations")
      .select("country_rule_id, status, notes, evaluator, earliest_legal_date, blocking_reason")
      .eq("case_id", id),
  ]);
  ...
  return (
    <>
      ...
      <LiveCaseView
        ...
        initialEvaluations={(evalRes.data ?? []) as EvaluationRow[]}
      />
    </>
  );
}
