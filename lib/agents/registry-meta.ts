...
export const AGENT_TYPE_BLURB: Record<AgentType, string> = {
  ...
  vet_network:
    "Matches the owner to an approved partner vet and books the procedure ladder: microchip implant, vaccinations, titer test, endorsement.",
  airline_crate:
    "Picks an IATA-LAR-compliant carrier + route, sizes the CR-82 crate, and clears temperature/breed embargoes for the corridor.",
  endorsement:
    "Runs the destination-specific endorsement window (USDA APHIS, MOCCAE, etc.) and tracks the courier handoff into the validity window.",
  comms:
    "Outbound owner communication. Citation-enforced WhatsApp + email; never invents requirements, always grounds in cited rules.",
  audit:
    "Read-only watchdog. Streams over agent outputs and flags citation gaps, deterministic-vs-LLM splits, SLA risk, low extraction confidence.",
  synthesizer:
